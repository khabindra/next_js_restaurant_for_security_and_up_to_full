...
export default function Testimonial() {
  ...
  const nextSlide = () => {
    ...
    setCurrentIndex((prev) => {
      const nextIndex = prev + 1;
      if (nextIndex === originalLength * 2) handleInfiniteLoopReset(originalLength);
      else {
        if (timeoutRef.current) clearTimeout(timeoutRef.current);
  ...
  const prevSlide = () => {
    ...
    setCurrentIndex((prev) => {
      const nextIndex = prev - 1;
      if (nextIndex === originalLength - 1) handleInfiniteLoopReset(originalLength * 2 - 1);
      else {
        if (timeoutRef.current) clearTimeout(timeoutRef.current);
        timeoutRef.current = setTimeout(() => setIsTransitioning(false), 600);
      }
  ...
  useEffect(() => {
    const measureWidth = () => {
      if (trackRef.current && trackRef.current.children.length > 1) setIsReady(true);
    };
    const timer = setTimeout(measureWidth, 50);
    window.addEventListener('resize', measureWidth);
    return () => {
  ...
  return (
    <section className="relative overflow-hidden bg-[#f9f8f6] py-32 md:py-48 border-t border-black/5">
      <div className="mx-auto w-full max-w-7xl px-6 md:px-12">

        <div className="text-center mb-20">
          <p className="text-[10px] font-medium uppercase tracking-[0.3em] text-stone-400 mb-6">Testimonials</p>
          <h2 className="text-3xl md:text-5xl font-serif font-extralight tracking-tight text-stone-900">
        ...
        <div className="relative flex items-center justify-center">
          ...
          <button 
            onClick={prevSlide} 
            className="absolute left-0 md:left-8 z-40 flex h-12 w-12 items-center justify-center text-stone-400 hover:text-stone-900 transition-colors duration-300 focus:outline-none focus-visible:ring-1 focus-visible:ring-stone-400" 
            aria-label="Previous testimonial"
          >
          ...
          <div 
            ref={containerRef}
            className={`w-full overflow-hidden transition-opacity duration-500 ${isReady ? 'opacity-100' : 'opacity-0'}`}
          >
            <div
              ref={trackRef}
            ...
            >
              {extendedTestimonials.map((testimonial, idx) => {
                ...
                return (
                  <article
                    key={`${testimonial.id}-${idx}`}
                    className={`relative flex w-[85vw] sm:w-[600px] md:w-[800px] flex-shrink-0 flex-col items-center text-center transition-all duration-700 ease-out ${
                      isCenter ? 'opacity-100' : 'opacity-0'
                    }`}
                  >
                    <span className="text-6xl md:text-8xl font-serif text-stone-200 leading-none mb-4 select-none pointer-events-none" aria-hidden="true">
                      &ldquo;
                    </span>
                    <p className="relative z-10 text-xl md:text-3xl font-serif font-light leading-[1.6] text-stone-800 mb-12 max-w-3xl">
                      {testimonial.quote}
                    </p>
                    <div className="relative z-10 flex flex-col items-center gap-1">
                      <span className="text-sm font-medium uppercase tracking-[0.2em] text-stone-900">
                        {testimonial.name}
                      </span>
          ...
          <button 
            onClick={nextSlide} 
            className="absolute right-0 md:right-8 z-40 flex h-12 w-12 items-center justify-center text-stone-400 hover:text-stone-900 transition-colors duration-300 focus:outline-none focus-visible:ring-1 focus-visible:ring-stone-400" 
            aria-label="Next testimonial"
          >
            <svg className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="1" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
        ...
        <div className="mt-16 flex justify-center gap-4">
          {testimonials.map((_, index) => (
            <button
              ...
              className={`h-px transition-all duration-500 focus:outline-none ${
                index === (currentIndex % originalLength) ? 'w-12 bg-stone-900' : 'w-6 bg-stone-300 hover:bg-stone-400'
              }`}
              aria-label={`Go to slide ${index + 1}`}
            />
          ))}
